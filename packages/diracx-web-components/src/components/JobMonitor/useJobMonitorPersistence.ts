...
export function loadInitialState(appId: string): JobMonitorState {
  ...
  return {
    ...
    // Row selection is deliberately not restored: persisted selections go
    // stale quickly and select-all would serialize hundreds of keys.
    rowSelection: {},
 ...
 * `rowSelection` is intentionally excluded (see loadInitialState).
 */
