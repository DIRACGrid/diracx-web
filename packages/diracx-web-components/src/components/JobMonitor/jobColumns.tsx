...
function StatusCell({ status }: { status: string }) {
  ...
  // statusColors is a module-level constant: no context read needed
...
export function validateAndConvertState(state: string): [string, boolean] {
  ...
  try {
    ...
    isValid =
      ...
      // rowSelection is no longer persisted, but may exist in older exports
      (parsed.rowSelection === undefined ||
  ...
  // `parsed` may be undefined/null when JSON.parse produced no usable
  // object (e.g. a non-SyntaxError failure or a literal "null").
  if (!parsed || typeof parsed !== "object") {
    throw new Error("The state is not a valid JSON object");
  }
