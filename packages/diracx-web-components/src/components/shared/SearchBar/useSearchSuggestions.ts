...
export function useSearchSuggestions({
  ...
  // Only retrigger the suggestions effect on input changes when the
  // consumer actually uses the current input (keeps the dependency array
  // a fixed length).
  const effectiveInput = usesCurrentInput ? inputValue : null;

  // Load suggestions (debounced)
  useEffect(() => {
    let cancelled = false;
    // Abort any in-flight suggestion request when the effect re-runs or
    ...
    const run = async () => {
      ...
      try {
        ...
        if (effectiveInput) {
          params.currentInput = effectiveInput;
        }
