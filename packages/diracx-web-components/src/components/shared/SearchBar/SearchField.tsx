...
export default function SearchField({
  ...
  useEffect(() => {
    setPlaceholder(
      previousToken
        ? previousToken.type.startsWith("operator")
          ? "Enter a value"
          : previousToken.type.startsWith("category")
            ? "Enter an operator"
            : "Enter a category"
        : "Enter a category",
    );
   ...
   * @param type The type of the token (e.g., "category", "custom", "operator", "value", ...).
   */
  function handleTokenCreation(label: string, type: string) {
    if (!allowKeyWordSearch && type === "custom") return;
    ...
    <Autocomplete
      ...
      onChange={(_e, value: string | null) => {
        if (value !== null && value !== "") {
          ...
          if (
            previousToken &&
            (previousToken.label === "is in" ||
              previousToken.label === "is not in")
          ) {
          ...
          const type = getTokenType(value.trim(), suggestions, previousToken);
          handleTokenCreation(value, type);
        }
      }}
    />
  );
