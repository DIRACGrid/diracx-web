...
export const PieChart = memo(function PieChart({
  ...
  // Keyboard path for the clickable legend entries (Enter/Space)
  ...
  const chartMargin = useMemo(() => ({ bottom: LEGEND_MARGIN }), []);
  ...
  const handleSliceClick = useCallback(
    (_event: unknown, pieItemIdentifier: { dataIndex: number }) => {
    ...
    <Box sx={{ position: "relative" }}>
      ...
      <Box
        sx={{
          ...
          position: "relative",
          // The legend overlaps the bottom band of the chart's svg box:
          // stack it explicitly above the chart surface so legend items
      ...
      >
        {pieData.map((item, index) => (
          <Box
            // Composite key: guards against callers passing duplicate ids,
            // which would otherwise corrupt list reconciliation and leave
            // orphaned legend entries behind.
            key={`${index}-${item.id}`}
            data-index={index}
            data-id={item.id}
            ...
            onKeyDown={onItemClick ? handleLegendKeyDown : undefined}
            role={onItemClick ? "button" : undefined}
            tabIndex={onItemClick ? 0 : undefined}
            aria-label={onItemClick ? `Filter by ${item.label}` : undefined}
            sx={{
              display: "flex",
              alignItems: "center",
              gap: 0.5,
