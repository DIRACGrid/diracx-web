...
async function generate() {
  ...
  const files = await fs.promises.readdir(DOCS_PATH);

  let generalUsage = "";

  for (const entry of files) {
    const fullPath = path.join(DOCS_PATH, entry);
    ...
    if (entry === "generalUsage.md") {
      ...
      const sections = [];
      const sectionFiles = await fs.promises.readdir(fullPath);
      for (const sectionFile of sectionFiles) {
        const sectionPath = path.join(fullPath, sectionFile);
